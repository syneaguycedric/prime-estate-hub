...
const PropertyCard = ({ id, title, price, location, type, surface, bedrooms, bathrooms, image, isNew, isFavorite = false }: PropertyCardProps) => {
    return (
            ...
            <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-2 bg-gradient-card border-border/50 cursor-pointer h-full flex flex-col">
                <div className="relative overflow-hidden rounded-t-lg">
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className={`absolute top-4 right-4 bg-card/90 hover:bg-card ${isFavorite ? "text-red-500" : "text-muted-foreground"}`}
                        onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                    ...
                    <div className="absolute bottom-4 left-4">
                        <div className="bg-primary text-primary-foreground px-3 py-1 rounded-lg font-semibold">{price}</div>
                    </div>
                </div>
